    ...
    <div className="mx-auto max-w-4xl p-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        {matches.map((match) => {
          ...
          return (
            <div key={match.id} className="rounded-2xl border border-line bg-surface p-3">
              <Link to={`/mascota/${match.pet.id}`}>
                {match.pet.fotos[0] && (
                  <img
              ...
              <Link
                to={`/matches/${match.id}/mensajes`}
                className="mt-2 block text-center text-sm font-medium text-forest"
              >
                Abrir conversación
              </Link>
            </div>
