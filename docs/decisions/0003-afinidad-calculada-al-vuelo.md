...
Esta decisión **sigue vigente y no se re-litiga** (`docs/integracion-adopcion.md` §5). Se traduce al stack actual así:

- El servicio se llamará `src/api/reencuentro_api/services/afinidad.py` (en la era Adopta era `services/affinity.py`); sigue siendo una función pura, sin persistir ningún score.
- El `Shelter` de aquella era es hoy `Organizacion`.
- Se implementa en la feature `AD-03`.

Este ADR se restauró a `main` el 2026-08-15 porque el pivot lo dejó únicamente en la rama `adopta-v1`, y el checkpoint 3 de `CHECKPOINTS.md` exige poder verificar que el código es consistente con el ADR que lo gobierna.
