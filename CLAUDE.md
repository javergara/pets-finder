...
- **`feature_list.json`** — el alcance real y el único que valida `init.sh`: **54 items** hoy (`python3 -c "import json;print(len(json.load(open('feature_list.json'))['items']))"`), del pivot (`01-pivot-fundaciones` …) a la fase 2 (`AD-01` … `AD-08`). Regla dura: **máximo un item `in_progress`**.
...
**50 items en `done` de los 54 de `feature_list.json`**, cada uno aprobado por un revisor independiente que corrió `bash init.sh` de verdad. Última corrida: **738 tests de Python + 487 de web, `Todo en verde.`** (`init.sh` no typechequea el frontend: `npx tsc -b --force` va aparte, a mano).
...
**Lo que está en la rama `feat/adoptar` y NO en producción**: el módulo de adopción completo (`AD-01`…`AD-07` en `done`, **`AD-08-integracion-transversal` en `in_progress`**). En `feature_list_adopcion.json` queda `AD-09-migraciones-y-deploy` en `todo`: es el que ejecuta las migraciones y verifica en prod.

⚠️ **El merge de `feat/adoptar` a `main` está BLOQUEADO**: hay **cuatro migraciones escritas y ninguna ejecutada**, en este orden obligatorio — `AD-03-swipes.sql` → `AD-03-home-profiles.sql` → `AD-05-matches.sql` → `AD-07-favorites.sql`. Con `SKIP_DB_CREATE_ALL=1` en producción no hay red de seguridad: si el código llega antes que las tablas, **cae la API entera**, no solo las pantallas nuevas. El estado por archivo vive en `migrations/README.md`.
