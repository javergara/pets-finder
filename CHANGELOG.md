...
## [3.0.0] - 2026-08-16 — **desplegado en producción el 2026-08-17** (PR #7)

Fase 2 del producto: **el módulo de adopción** (`/adoptar`), que cierra el arco de la emergencia con la mascota que nadie reclama. Cubre las ocho features `AD-01`…`AD-08` de `feature_list_adopcion.json`; `AD-09` (migrar, mergear y verificar en producción) cerró el 2026-08-18 con el flujo completo verificado en petfinder-col.com (evidencia en `progress/evidencia-ad09-prod.txt`).

✅ **Este release ESTÁ en producción.** `AD-01`+`AD-02` salieron con el 2.5.0 (tabla `pets` migrada el 2026-08-15); las otras cuatro migraciones se ejecutaron el 2026-08-16 —con autorización explícita del dueño y ANTES del merge— en este orden: `AD-03-swipes.sql` → `AD-03-home-profiles.sql` → `AD-05-matches.sql` → `AD-07-favorites.sql`, verificadas contra `information_schema` con RLS y constraints. El merge a `main` (`586b00b`, PR #7) disparó el deploy el 2026-08-17. La regla sigue vigente para el futuro: con `SKIP_DB_CREATE_ALL=1` en producción, si el código llega antes que las tablas **cae la API entera** — guía de referencia para entornos nuevos: `docs/despliegue-modulo-adopcion.md`.
...
Suite al cerrar el release (tras el merge del PR #7): **753 tests de Python + 493 de web**, `bash init.sh` en verde. Al abrir `AD-01` eran 174 + 148 (ese tramo incluye también trabajo ajeno al módulo: feature 47 y los PRs 4 y 5).
...
- **Esquema — cinco tablas nuevas**: `pets` (AD-01, migrada el 2026-08-15), `swipes` y `home_profiles` (AD-03), `matches` (AD-05) y `favorites` (AD-07) — las cuatro últimas **ejecutadas en prod el 2026-08-16**, todas con RLS. Cada `.sql` tiene su test anti-drift (`tests/api/test_migracion_*.py`, cuatro) que lo compara columna a columna contra el modelo y exige que viajen el RLS y las constraints con su nombre.
